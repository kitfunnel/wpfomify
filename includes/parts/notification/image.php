...
<div class="ibx-notification-popup-img<?php echo empty( $img ) ? ' has-letter' : ''; ?>">
	<?php if ( ! empty( $img ) ) { ?>
	<img src="<?php echo esc_url( $img ); ?>" alt="<?php echo isset( $value['title'] ) ? htmlspecialchars( $value['title'] ) : ''; ?>" width="<?php echo $settings->img_size; ?>" height="<?php echo $settings->img_size; ?>" />
	<?php } else { ?>
		<?php
		echo $name;
		?>
